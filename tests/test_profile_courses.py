...
class ProfileAndCourseTests(unittest.TestCase):
    ...
    def test_learning_engine_starts_from_published_stage_and_advances_sequentially(self) -> None:
        ...
        self.assertEqual(path.json()["total_stage_count"], 40)
        self.assertEqual(len(path.json()["stages"]), 40)
        self.assertEqual(path.json()["module_count"], 5)
        self.assertEqual(path.json()["final_exam_required"], True)
        ...
        self.assertEqual(lesson.json()["module_stage_count"], 8)
        self.assertTrue(lesson.json()["content"]["ui_hint"]["avatar_visible"])
        self.assertEqual(blocked.status_code, 409)
        self.assertEqual(completion.status_code, 200)
        self.assertEqual(completion.json()["progress_percentage"], 2)
        ...
        self.assertEqual(enrollment.progress_percentage, 2)
        self.assertEqual(enrollment.current_stage_number, 2)
        self.assertEqual(len(progress_rows), 40)
