param(
    ...
    [ValidateSet("init", "import-env", "set", "set-server-password", "list", "diagnose-sms", "migrate-db", "seed-course-content", "audit-db", "sync-mock-kb", "verify-rag", "verify-coach", "run-rag-indexer", "run-server")]
    [string]$Action,
...
switch ($Action) {
    ...
    "seed-course-content" {
        ...
        Write-Output "vault-environment-loaded=$loaded"
        & $python -m src.cli.seed_course_content
        exit $LASTEXITCODE
    }
    "audit-db" {
        $loaded = Import-VaultEnvironment
